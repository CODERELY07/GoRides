...
const checkExistingRideRequest = async () => {
  ...
  const acceptRide = async (rideId) => {
    try {
      db = await SQLite.openDatabaseAsync('goRides');
      await db.runAsync('UPDATE rides SET ride_status = "accepted", rider_name = ? WHERE id = ?', [username, rideId]);
  ...
  const markRideAsComplete = async () => {
    try {
      ...
      // Navigate to the RideHistory screen after completion
          ...
          <View style={styles.header}>
            <View style={styles.inlineContainer}>
              ...
              <View style={styles.profileTextHolder}>
                ...
                <Text style={[styles.smallText, { color: "#fff" }]}>Let's Ride</Text>
              </View>
            </View>
